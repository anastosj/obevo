...
public class PostgresqlMetadataDialect extends AbstractMetadataDialect {
    ...
    public ImmutableSet<DaExtension> getExtensionsOptional(Connection conn) throws SQLException {
        ...
        return maps.<DaExtension>collect(map -> new DaExtensionImpl((String) map.get("EXTNAME")))
                .toSet()
                .toImmutable();
    }
    ...
    public ImmutableCollection<DaUserType> searchUserTypes(final DaSchema schema, Connection conn) {
        String sql = """
                SELECT t.typname\s
                FROM        pg_type t\s
                LEFT JOIN   pg_catalog.pg_namespace n ON n.oid = t.typnamespace\s
                WHERE       (t.typrelid = 0 OR (SELECT c.relkind = 'c' FROM pg_catalog.pg_class c WHERE c.oid = t.typrelid))\s
