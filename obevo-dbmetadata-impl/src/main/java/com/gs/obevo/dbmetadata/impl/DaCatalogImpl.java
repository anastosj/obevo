...
public class DaCatalogImpl implements DaCatalog {
    ...
    public ImmutableCollection<DaTable> getTables() {
        return CollectionAdapter.adapt(delegate.getTables())
                .<DaTable>collect(object -> {
                    if (object instanceof View view) {
                        return new DaViewImpl(view, schemaStrategy, extraViewInfoMap.get(view.getName()));
                    } else {
    ...
    public ImmutableCollection<DaRoutine> getRoutines() {
        return extraRoutines
                .newWithAll(CollectionAdapter.adapt(delegate.getRoutines()).collect(object -> new DaRoutine6Impl(object, schemaStrategy, routineOverrideValue)));
    }

    @Override
    public ImmutableCollection<DaSequence> getSequences() {
        return CollectionAdapter.adapt(delegate.getSequences()).<DaSequence>collect(object -> new DaSequence6Impl(object, schemaStrategy)).toImmutable();
    }
    ...
    public ImmutableCollection<DaSynonym> getSynonyms() {
        return CollectionAdapter.adapt(delegate.getSynonyms()).<DaSynonym>collect(object -> new DaSynonym6Impl(object, schemaStrategy)).toImmutable();
    }
}
