...
public class AseToH2SqlTranslator implements PostColumnSqlTranslator, PostParsedSqlTranslator {
    private static final Pattern ALTER_TABLE_ALTER_COLUMN_PATTERN = Pattern.compile("(?i)(alter\\s+table\\s+\\w+\\s+alter\\s+column\\s+\\w+\\s+)(.+)");
    private static final Pattern NULL_OR_NOT_NULL_PATTERN = Pattern.compile("(?i)(NOT\\s+NULL|NULL)(.*)");
    ...
    public String handleAnySqlPostTranslation(String string, ChangeInput change) {
        ...
        var alterTableAlterColumnMatcher = ALTER_TABLE_ALTER_COLUMN_PATTERN.matcher(string);
        if (alterTableAlterColumnMatcher.find()) {
            ...
            if (!StringUtils.containsIgnoreCase(alterTableAlterColumnMatcher.group(2), "set") &&
                ...
                var nullOrNotNull = NULL_OR_NOT_NULL_PATTERN.matcher(alterTableAlterColumnMatcher.group(2));
